            ...
            for (int column = 0; column < width; column++)
            {
                switch (_rules)
                {
                    ...
                    case Rules.SeedsLight:
                        if (_field[column, row] is 1 or 2)
                        {
                            newField[column, row] = _field[column, row]++;
                        }
                        else
                        {
                            var neighbor = GetNeighbor(column, row);
                            if (neighbor.IndexOf("..") < 0 && !(neighbor[0] == '.' && neighbor[7] == '.') && GetNeighborCount(column, row) == 2)
                            {
                                newField[column, row] = 1;
                            }
                        }
    ...
    public void Evolution2(object? sender, EventArgs args)
    {
        ...
        for (int row = 0; row < height; row++)
        {
            for (int column = 0; column < width; column++)
            {
                ...
                {
                    if (_birth.Contains(GetNeighborCount4(column, row).ToString()) && _field[column, row] == 0)
                    {
                        newField[column, row] = 1;
                    }
                }
